...
Pert = QuanFloq.Models.TwoLevel(w0=w0,w=w,k_max=100,v=v,xi=xi,...
    RWA=true,rand_v=false);
Pert2 = QuanFloq.Models.TwoLevel(w0=w0,w=w,k_max=100,v=v,xi=tol,...
    RWA=true,rand_v=false);
V = sqrt(w^2-del^2);
Unpert.V = V;
Pert.V = V;
...
Details = QuanFloq.Details(script='Calc_TwoLevel2.m',...
...
fprintf('(Pert) Doing AE variational principle:\n');tic;
Res_Pert2_AE = Pert2.variational('AE',Psi0=Psi0,...
    TrackPsi=true,filter_nconv=false,tol=tol);
fprintf('(Pert) Done AE method: %f (s)\n',toc);
%% PostProcess
