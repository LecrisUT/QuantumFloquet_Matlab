classdef HubardDimerOrbital < QuanFloq.baseFloquetHF
    % HubardDimerOrbital Driven Hubard dimer single Slater approximation
    % See QuanFloq.HubardDimerExact for the Hamiltonian.
    ...
    % 
    %
    % See also QuanFloq.Models.HubardDimerExact
    ...
    methods
        function obj = HubardDimerOrbital(Args,Args2)
            ...
            obj@QuanFloq.baseFloquetHF(2,Args2{:},Ne=2,mode='closed-shell');
            obj.t = Args.t;
            obj.v0 = Args.v0;
            obj.v1 = Args.v1;
        end
    ...
    methods
%         function newobj = QuanFloq.Models.HubardDimerExact(obj)
% %             newobj = QuanFloq.Models.HubardDimerExact('v0');
%         end
        function Psi_Slat = OrbitalToSlater(obj,Psi_Orb,Args)
            arguments
                obj         QuanFloq.HubardDimerOrbital
                Psi_Orb     double
                Args.objSlater          (1,1)   QuanFloq.HubardDimerExact
            ...
            % Slater determinant basis corresponds to that in QuanFloq.HubarDimerExact
            %
            % See usage of QuanFloq.baseFloquetHF.OrbitalToSlater
            %
    ...
    methods
        function json = jsonencode(obj,varargin)
            j = jsonencode@QuanFloq.baseFloquet(obj);
            S = jsondecode(j);
    ...
    methods (Access=protected)
        function groups = getPropertyGroups(obj)
            ...
            groups = getPropertyGroups@QuanFloq.baseFloquetHF(obj);
            if isscalar(obj)
