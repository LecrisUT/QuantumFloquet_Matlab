classdef baseSystemBath < QuanFloq.baseCalc
    ...
    properties
        ...
        system  QuanFloq.baseFloquet    {mustBeScalarOrEmpty}
        % bath - Bath being coupled
        % See also baseBath
        bath    QuanFloq.baseBath       {mustBeScalarOrEmpty}
    ...
    properties (Abstract)
        ...
        %
        % See also QuanFloq.baseFloquet.N, QuanFloq.baseBath.nu
        V       (:,:)   double
    end
    properties (Dependent)
        % hk_range - Helper property for [-hk_max:hk_max]
        % See also QuanFloq.baseSystemBath.hk_max
        hk_range
        % hk_max2 - Helper property for 2*hk_max+1
        % See also QuanFloq.baseSystemBath.k_max
        hk_max2
    end
    methods
        function obj = baseSystemBath(system,bath,Args)
            arguments
                system  (1,1)   QuanFloq.baseFloquet
                bath    (1,1)   QuanFloq.baseBath
